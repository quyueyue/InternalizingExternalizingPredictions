...
do
    ...
    if [ "$cluster" == "none" ];then
        ...
        sbatch --job-name=${testLPname} --partition=psych_day --time=20:00:00 --mem-per-cpu=24G --wrap="${scripts_dir}/CBIG_TRBPC_PFM_job_HCP.sh $input_dir $feature_file $score_ind $outdir $model"
    fi
done

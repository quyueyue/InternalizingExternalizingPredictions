...
HCP_KRR_PFM_dir='/gpfs/milgram/project/holmes/yq222/FYP/HCP/HCP_output_KRR/'
...
adult_int_ind = np.where(HCP_varnames=='ASR_Intn_Raw')[0][0]
adult_ext_ind = np.where(HCP_varnames=='ASR_Extn_Raw')[0][0]
adult_anx_dep_ind=np.where(HCP_varnames=='ASR_Anxd_Raw')[0][0]
adult_witd_dep_ind=np.where(HCP_varnames=='ASR_Witd_Raw')[0][0]
adult_rulebreak_ind = np.where(HCP_varnames=='ASR_Rule_Raw')[0][0]
...
exact_test_dir = os.path.join(HCP_KRR_PFM_dir,'exact_test')
...
np.savetxt(os.path.join(HCP_KRR_PFM_dir,'exact_test/sig_edge_bin_int_ext.csv'),sig_edge_bin_int_ext)

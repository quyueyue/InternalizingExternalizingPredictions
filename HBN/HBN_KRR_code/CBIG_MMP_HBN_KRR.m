function CBIG_MMP_HBN_KRR(outerFolds, innerFolds, feature_path, ...
    split, outdir, subtxt, scorecsv, predvar, covtxt, ymat, covmat)
...
% feature details
outstem = convertStringsToChars("KRR_HBN"); 
% feature_name = featurebase;
param.outstem = outstem;
param.outdir = fullfile(outdir, seed_name, 'results');
% subject details
sub_txt = subtxt;
score_csv = {scorecsv}; 
...
if ~exist(fullfile(outdir, y_mat))
    ...
    y = CBIG_read_y_from_csv(score_csv, 'Identifiers', score_names, score_types,...
...
if ~exist(fullfile(outdir,cov_mat))
    ...
    cov = CBIG_generate_covariates_from_csv(score_csv, 'Identifiers', cov_names, cov_types, ...
...
% load features
features = load(strcat(feature_path));
feature_name = fieldnames(features);
param.feature_mat = features.(feature_name{1});

function hyp_driven_mask = CBIG_TRBPC_compute_PFM_significance_mask_HBN_wrapper(perm_dir, outdir)
...
HBN_project_code_dir = fullfile(getenv('IntExt_DIR'),'HBN');
addpath(genpath(project_code_dir));
addpath(genpath(HBN_project_code_dir));

%% set common variables
% repdata_dir = fullfile(getenv('CBIG_REPDATA_DIR'),'stable_projects','predict_phenotypes','ChenTam2021_TRBPC');
PFM_dir = fullfile(getenv('IntExt_DIR'),'HBN','HBN_old_output_KRR');
...
if ~exist(fullfile(PFM_dir,'PFM_network_mean.mat'),'file')
    PFM_network = CBIG_TRBPC_compute_network_mean_PFM_HBN(PFM_dir, N_score, N_state, PFM_dir);
else
...
behav_cat{1} = 1; % adolescent internalizing
behav_cat{2} = 4; % child externalizing

...
do
    ...
    date >> $LF
    matlab -nodesktop -nosplash -nodisplay -r " addpath $scripts_dir; CBIG_TRBPC_prepare_PFM_perm_inputs_HBN( \
    ...
    do
        if [ "$cluster" == "none" ];then
            ...
            ${scripts_dir}/CBIG_TRBPC_PFM_perm_job_HBN.sh $FC_mean_file $y_file $krr_folds $perm_seed_start $perm_num $outdir $family_list $subject_list $seed_idx
        else
            ...
            cmd="${scripts_dir}/CBIG_TRBPC_PFM_perm_job_HBN.sh $FC_mean_file $y_file $krr_folds $perm_seed_start $perm_num $outdir $family_list $subject_list $seed_idx"
        sbatch --job-name=PFM_perm --partition=psych_day --time=24:00:00 --mem-per-cpu=30G --wrap="${scripts_dir}/CBIG_TRBPC_PFM_perm_job_HBN.sh $FC_mean_file $y_file $krr_folds $perm_seed_start $perm_num $outdir $family_list $subject_list $seed_idx"
        fi
    done
done

function CBIG_TRBPC_compute_PFM_permutation_stats_HBN(FC_mean_file, y_file, krr_folds,...
    perm_seed_start, perm_num, site_list, subject_list, outdir, seed_idx)
...
project_code_dir = fullfile(getenv('IntExt_DIR'),'ABCD'); % we need some code under ABCD/utilities
...
[~, loc] = ismember(subjects,sub_table.Identifiers); % find indices in the sub_table (1113 subjs in total) corresponding to the subset of 752 subjects used for analysis
site_all = sub_table.Site(loc);
[~, ~, site_ind] = unique(site_all); % index of family ID of each subject in site_all (for eg, subject 1's family ID is the 361th family ID in site_all)
N_site = max(site_ind); % 385 families in total
for i = 1:N_site
...
for i = 1:perm_num
    ...
    perm_group = zeros(length(site_ind),1); % coding for which families are included in the permutation
    rng(i+perm_seed_start-1);
    for k = 1:N_site
        sorting_order = datasample([1:max(site_count)],site_count(k),'Replace', false); % max(site_count)=5; site_count(k) random samples of 1:5;
        % site_count(k)=number of subjects who belong to the kth
        % family ID
        ...
        perm_group(find(site_ind == k)) = sorting_order; % perm_group(find(site_ind == k)) extracts the perm_group indices for subjects belonging to the kth family
        % perm_group is a 1*752 vector
    end
    % permute within groups
    perm_ind = 1:length(site_all);
    for n = 1:max(site_count) % equivalent to: for perm_group_id = 1:5

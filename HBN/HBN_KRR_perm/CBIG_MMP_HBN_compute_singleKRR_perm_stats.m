function CBIG_MMP_HBN_compute_singleKRR_perm_stats(singleKRR_dir, outstem, score_ind, ...
 perm_seed_start, N_perm, site_list, subject_list, outdir)
...
%
%   - subject_list
%     A text file where each line is the subject ID to be extracted from site_list.
%
%   - outdir
...
[~, loc] = ismember(subjects,sub_table.Identifiers);
site_all = sub_table.Site(loc);
[~, ~, site_ind] = unique(site_all);
N_sites = max(site_ind);
for i = 1:N_sites
	site_count(i,:) = sum(site_ind == i);
end
...
for h=1:N_seeds
	...
	if ~exist(outfile)
		for i = 1:N_fold
			...
			y_perm = y_resid;
			% split into max(site_count) groups
			perm_group = zeros(length(site_ind),1);
			rng(j+perm_seed_start-1);
			for k = 1:N_sites
			    sorting_order = datasample([1:max(site_count)],site_count(k),'Replace', false);
			    perm_group(find(site_ind == k)) = sorting_order;
			end
			% permute within groups
			for n = 1:max(site_count)

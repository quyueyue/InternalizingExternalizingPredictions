...
#        errfile=${outdir}/job_err_out/testloop.err
        ...
        sbatch --job-name=${testLPname} --partition=psych_day --time=2:00:00 --mem-per-cpu=8G --wrap="${root_dir}/CBIG_TRBPC_KRR_LpOCV_testloop.sh -p ${paramfile} -n ${num_test_folds} -o ${outdir}"

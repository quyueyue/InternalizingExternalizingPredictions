...
elif [ "$stage" == "gather" ]; then

        paramfile=${outdir}/param.mat
        kernel_folders=${outdir}/kernel_folders.mat
        num_test_folds=`cat ${outdir}/num_test_folds.txt`

	echo "gather results of all cross-validation test folds"
...
usage() { echo "
NAME:
	CBIG_TRBPC_multiKRR_LpOCV_workflow_slurm.sh
...
REQUIRED ARGUMENTS:
	...
	-stage 		   stage            : run either "submitloop" or "gather" stage of the pipeline
...
Example:
	bash $IntExt_Dir/ABCD/ABCD_KRR_code/multiKRR/CBIG_TRBPC_multiKRR_LpOCV_workflow_slurm.sh -csv_file $IntExt_Dir/ABCD/ABCD_data_inputs/ABCD_alldata.csv 

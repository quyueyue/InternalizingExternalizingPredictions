...
#	cmd="$cmd  $FC_mean_file $y_file $krr_folds $perm_seed_start $perm_num $outdir $group"
    ...
    sbatch --job-name=PFM_perm --partition=psych_day --time=12:00:00 --mem-per-cpu=16G --wrap="${scripts_dir}/CBIG_TRBPC_PFM_perm_job_multiKRR.sh $FC_mean_file $y_file $krr_folds $perm_seed_start $perm_num $outdir $group"

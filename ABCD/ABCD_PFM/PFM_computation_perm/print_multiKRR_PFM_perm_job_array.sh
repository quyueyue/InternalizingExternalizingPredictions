...
# Usage
usage() { echo "
Usage: $0 -i <input_dir> -a <start_behav_ind> -b <end_behav_ind> -s <outstem> -n <N_per_job> -t <total_num> -o <outdir> -m <model>
-g <group> [-c <cluster>]
    ...
    - start_behav_ind The starting index of the behavioral measure in the regression model
    - end_behav_ind  The ending index of the behavioral measure in the regression model
...
while getopts ":i:f:a:b:s:n:t:o:m:g:c:" opt; do
    case "${opt}" in
        i) input_dir=${OPTARG};;
        f) feature_file=${OPTARG};;
        a) start_behav_ind=${OPTARG};;
        b) end_behav_ind=${OPTARG};;
        s) outstem=${OPTARG};;
        n) N_per_job=${OPTARG};;
        t) total_num=${OPTARG};;
        o) outdir=${OPTARG};;
...
shift $((OPTIND-1))
if [ -z "${input_dir}" ] || [ -z "${feature_file}" ] }|| [ -z "${start_behav_ind}"] || [ -z "${end_behav_ind}" ] || [ -z "${outstem}" ] || [ -z "${model}" ] || \
...
for score in `seq ${start_behav_ind} ${end_behav_ind}`
do
...
#            cmd="${cmd} ${model} ${input_dir} ${outstem} ${score} ${perm_start} ${N_per_job} ${group} ${outdir}"
            ...
            echo ${cmd}>>${scripts_dir}/jobs_multiKRR_PFM_perm_${start_behav_ind}-${end_behav_ind}.txt
